		...
		fetchDataFromApi(
			...
			.then((response) => {
				...
				if (response && response.data) {
					flag
						? setResturantsList([...ResturantsList, ...response?.data?.cards])
						: // ? setResturantsList([...ResturantsList, ...response?.data?.cards])
						  setResturantsList(response.data.cards);
				...
				<div className=" max-w-[1200px] min-w-[1200px] my-0 mx-auto relative block">
					<div className="mt-0 mx-auto py-[20px] bg-white">
						<div className="relative  h-[72px]">
							<div
								className="flex items-center translate-x-[10px] translate-y-[18px] absolute top-0 left-0 right-0 "
								id="myHeader"
							>
								<div className=" flex items-center my-0 mx-auto max-w-[1200px] min-w-[1200px] h-[100%]">
									<h3 className=" font-[600] text-[#282c3f] text-[28px] flex-1  h-[100%] flex items-center">
										{ResturantsList.length} restaurants
									</h3>
									<Filter />
								</div>
							</div>
